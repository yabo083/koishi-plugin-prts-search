...
- **卡片风格**（`cardStyle`）：
  ...
  - `weekly` 泰拉周刊（夜间书脊）：深底 + 立绘光晕 + 磨砂玻璃分区卡 + 左侧强调色书脊。每次渲染从已下载的生日干员 PNG 立绘实时提取书脊和星期徽章颜色，生日区衬影使用实际立绘；无需预置干员色表，新干员随图片自动支持。多位生日干员取首个可提取颜色的立绘，无有效颜色时回退星期色；其他区域保持星期轮换色。版面按「群聊压到 400px 宽仍可读」定尺，正文不小于 26px。当日生日干员恰好一位时，封面展示「干员报到」台词（取自 PRTS 语音记录，字号按台词长度自适应；抓取失败回退生日贺语）。
