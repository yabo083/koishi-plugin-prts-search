...
- **卡片风格**（`cardStyle`）：
  ...
  - `weekly` 泰拉周刊（夜间书脊）：深底 + 立绘光晕 + 磨砂玻璃分区卡 + 左侧强调色书脊，强调色按星期轮换；当天有生日干员且主题色表查得到时，书脊/星期徽章/生日区衬影改为该干员的立绘主题色。版面按「群聊压到 400px 宽仍可读」定尺，正文不小于 26px。当日生日干员恰好一位时，封面改为展示该干员的「干员报到」台词（取自 PRTS 语音记录，字号按台词长度自适应；抓取失败回退原生日贺语）。
